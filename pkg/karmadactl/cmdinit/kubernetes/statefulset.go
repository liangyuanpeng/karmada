...
func (i *CommandInitOption) etcdInitContainerCommand() []string {
	...
	for v := int32(0); v < i.EtcdReplicas; v++ {
		etcdClusterConfig += fmt.Sprintf("%s-%v=http://%s-%v.%s.%s.svc.%s:%v", etcdStatefulSetAndServiceName, v, etcdStatefulSetAndServiceName, v, etcdStatefulSetAndServiceName, i.Namespace, i.ServiceDNSDomain, etcdContainerServerPort) + ","
	}
...
`,
			...
			etcdEnvPodIP, etcdContainerServerPort,
			etcdEnvPodName, etcdStatefulSetAndServiceName, i.Namespace, i.ServiceDNSDomain, etcdContainerClientPort,
