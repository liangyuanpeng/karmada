...
func (i *CommandInitOption) etcdServers() string {
	...
	for v := int32(0); v < i.EtcdReplicas; v++ {
		etcdClusterConfig += fmt.Sprintf("https://%s-%v.%s.%s.svc.%s:%v", etcdStatefulSetAndServiceName, v, etcdStatefulSetAndServiceName, i.Namespace, i.ServiceDNSDomain, etcdContainerClientPort) + ","
	}
...
func (i *CommandInitOption) karmadaAPIServerContainerCommand() []string {
	return []string{
		...
		fmt.Sprintf("--service-account-issuer=https://kubernetes.default.svc.%s", i.ServiceDNSDomain),
